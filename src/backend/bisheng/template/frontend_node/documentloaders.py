...
def build_file_field(suffixes: list, fileTypes: list, name: str = "file_path", fieldType="fileNode") -> TemplateField:
...
class DocumentLoaderFrontNode(FrontendNode):
    ...
    file_path_templates: ClassVar[Dict] = {
        "ElemUnstructuredLoaderV0": build_file_field(
            suffixes=[
                ".html",
                ".md",
                ".txt",
        ...
        "PyPDFLoader": build_file_field(suffixes=[".pdf"], fileTypes=["pdf"], fieldType="fileNode"),
        ...
        "UniversalKVLoader": build_file_field(
            suffixes=[".jpg", ".png", ".jpeg", ".bmp", ".pdf"],
            fileTypes=["jpg", "png", "jpeg", "bmp", "pdf"],
        ),
        "CustomKVLoader": build_file_field(
            suffixes=[
                ".jpg",
                ".png",
                ".jpeg",
            ...
            fileTypes=["jpg", "png", "jpeg", "pdf", "txt", "docx", "doc", "bmp", "tif", "tiff", "xls", "xlsx"],
        ),
        "CustomPDFLoader": build_file_field(suffixes=[".pdf"], fileTypes=["pdf"]),
    }
    ...
    def add_extra_fields(self) -> None:
        ...
        if self.template.type_name in {"GitLoader"}:
            ...
            self.template.add_field(
                TemplateField(
                    field_type="int", required=True, show=True, name="request_timeout", advanced=True, value=600
                )
            )
            self.template.add_field(self.file_path_templates[self.template.type_name])
        elif self.template.type_name in {"CustomPDFLoader"}:
            self.template.add_field(
                TemplateField(
...
def build_pdf_semantic_loader_fields():
    ...
    file_path = TemplateField(
        field_type="file", required=True, show=True, name="file_path", value="", display_name="pdf文件"
    )
    password = TemplateField(
        field_type="str", required=True, show=True, advanced=True, name="password", value=None, display_name="password"
    )
    layout_api_key = TemplateField(
        field_type="str", required=False, show=True, name="layout_api_key", value=None, display_name="layout_api_key"
    )
    layout_api_url = TemplateField(
        field_type="str", required=False, show=True, name="layout_api_url", value=None, display_name="layout_api_url"
    )
    ...
    start = TemplateField(
        field_type="int", required=True, show=True, advanced=True, name="start", value=0, display_name="start"
    )
    n = TemplateField(
        field_type="int", required=False, show=True, advanced=True, name="n", value=None, display_name="n"
    )
